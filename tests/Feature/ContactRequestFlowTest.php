...
class ContactRequestFlowTest extends TestCase
{
    ...
    public function test_valid_message_is_stored_notified_and_redirected_to_truthful_confirmation(): void
    {
        ...
        Mail::assertSent(ContactRequestConfirmation::class, function ($mail): bool {
            return $mail->hasTo('avery@example.com')
                && $mail->envelope()->subject === 'We received your message'
                && str_contains($mail->render(), 'Hello Avery Owner,')
                && str_contains($mail->render(), 'I have a general question about Local Works.')
                && str_contains($mail->render(), '555-0100');
        });
    ...
    public function test_internal_mail_failure_does_not_discard_message_or_prevent_confirmation(): void
    {
        $confirmation = Mockery::mock();
        $confirmation->shouldReceive('send')->once()->with(Mockery::type(ContactRequestConfirmation::class));
    ...
    public function test_confirmation_mail_failure_does_not_discard_message_or_prevent_internal_notification(): void
    {
        $notification = Mockery::mock();
        $notification->shouldReceive('send')->once()->with(Mockery::type(NewContactRequestNotification::class));
